    ...
    def save_to_db(self, valid_ips):
        ...
        try:
            for item in valid_ips:
                # 插入数据
                print(item)
                sql='INSERT INTO %s VALUES("%s", 1, 0, 0, 1.0, 2.5)' %(cfg.TABLE_NAME, item)
                print(sql)
                ...
                # 输出入库状态
                if n:
                    print datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" "+item+" yes\n"
                else:
                    print datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" "+item+" no\n"

            else:
                print datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" "+ item +" exist\n"
        except Exception as e:
            print "error：" + str(e)
        finally:
            cursor.close()
